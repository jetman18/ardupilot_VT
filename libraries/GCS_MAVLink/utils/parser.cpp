...
#include "parser.h" 
#include <cstdio>  // thêm dòng này
#include <cstring>
#include "string.h"

#define STRNCMP strncmp

parser::parser(void) : _lastToken(-1),_currToken(0)
{
    _inputStr[0] = 0;
    _tokens[0] = NULL;
};
...
*/
const char* parser::getToken(int index) const
{
    if (index <= _lastToken)
...
int parser::getTokenAsInt(int index)
{
	int val = -1;
	if(index <= _lastToken && str_to_int(_tokens[index], &val))
		return val;
	else
		return -1;
}
...
*/
bool parser::loadLine(const char* str, int ntok, const char* delimiters)
{
...
*/
int parser::getTokenPosition(const char *token) const
{
	size_t tLen = strlen(token);
	for(int i = 0; i <= _lastToken; i++)
	{
		if(STRNCMP(_tokens[i], token, tLen) == 0)
			return i;
	}
	return -1;
}
...
*/
int parser::getTokenIndex(const char *token, const char *str)
{
	size_t tLen = strlen(token);
...
*/
int	parser::getTokenIndex(int token) const
{
...
int parser::_atoi(const char *str) {
    int result = 0;
    int sign = 1;
...
bool parser::str_to_int(const char *str, int *out_value) {
    int result = 0;
    int sign = 1;
